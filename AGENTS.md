...
- GitHub profile images are fetched through camo and can return `504` when the first origin response is slow. Cache fully rendered SVG responses in Redis so normal `/api/card` requests can serve baked cards quickly instead of recomputing provider/artwork/render work for GitHub. SVG responses should also set shared-cache directives such as `s-maxage` so Vercel/CDN can satisfy camo origin fetches before Serverless cold starts or Redis latency matter. Preserve active-session short cache windows for the visible timer; do not serve baked/backup SVGs for timer cards because static SVG cache layers can diverge. Keep a longer backup SVG cache only for non-timer error fallback.
