...
- GitHub profile images are fetched through camo and can return `504` when the first origin response is slow. Cache fully rendered SVG responses in Redis so normal `/api/card` requests can serve baked cards quickly instead of recomputing provider/artwork/render work for GitHub.
