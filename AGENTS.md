...
- Remote image embedding uses two cache layers: `image-data:<hash(candidateUrl)>` stores data URIs, while `image-candidate:<hash(sourceUrl,purpose,size)>` stores the last successful resized URL for 7 days so known-bad 4xx resize variants are not retried first.
- GitHub profile images are fetched through camo and can return `504` when the first origin response is slow. Cache fully rendered SVG responses in Redis so normal `/api/card` requests can serve baked cards quickly instead of recomputing provider/artwork/render work for GitHub. SVG responses should also set shared-cache directives such as `s-maxage` so Vercel/CDN can satisfy camo origin fetches before Serverless cold starts or Redis latency matter. Preserve active-session short cache windows for the visible timer; do not serve baked/backup SVGs for timer cards because static SVG cache layers can diverge. Timer card HTTP responses should use explicit `no-cache` revalidation headers so GitHub camo does not keep a stale static timer. Keep a longer backup SVG cache only for non-timer error fallback. To ensure GitHub camo refreshes promptly and does not lock onto stale images, decouple HTTP Cache-Control from upstream provider TTLs, keep non-timer `s-maxage` and `stale-while-revalidate` short (e.g., 30s), and use full `no-cache, no-store, must-revalidate, max-age=0` headers for active session/timer cards.
- Keep card performance diagnostics low-noise and request-scoped. The `[perf] card` log should expose total time plus major phases such as presence cache, SVG cache, OpenXBL, title art, image embedding, session/history Redis operations, render, and SVG cache write so Vercel/GitHub camo timeouts can be traced without guessing.
- Upstash/remote Redis round trips can dominate card latency. When updating independent state such as last-seen, session reads, current-session markers, and related writes, keep correctness dependencies explicit but batch independent Redis operations with `Promise.all` rather than serial `await`s.
- SVG card typography must not depend on Vercel or GitHub viewer system fonts. Keep card fonts in `fonts/` and render visible card text as SVG paths when stable visual layout matters, because SVGs embedded through `<img>` can apply fonts differently from standalone SVG documents. To ensure Vercel NFT (Node File Trace) correctly bundles these font files, avoid passing dynamic variables to `new URL(path, import.meta.url)`. Instead, construct them using static string literals so Vercel can statically trace and copy the ttf files to the serverless bundle, and ensure `vercel.json` has explicit `includeFiles` backup rules.
